...
namespace MagicLeap
{
    ...
    /// </summary>
    public class SimulatorHandTracking : MonoBehaviour
    {
...
#pragma warning restore 414
        ...
        public GameObject thumb_1, index_1;
...
//#endif
            ...
            for (int i = 0; i < Hand.Index.KeyPoints.Count; ++i)
            {
                if (i == 2)
                {
                    _indexFinger.Add(CreateKeyPoint(Hand.Index.KeyPoints[i], _indexColor, 2).transform);
                } else
                {
                    _indexFinger.Add(CreateKeyPoint(Hand.Index.KeyPoints[i], _indexColor, 0).transform);
                }
            ...
            for (int i = 0; i < Hand.Thumb.KeyPoints.Count; ++i)
            {
                if (i == 2)
                {
                    _thumb.Add(CreateKeyPoint(Hand.Thumb.KeyPoints[i], _thumbColor, 1).transform);
                } else
                {
                    _thumb.Add(CreateKeyPoint(Hand.Thumb.KeyPoints[i], _thumbColor, 0).transform);
                }
            ...
            for (int i = 0; i < Hand.Wrist.KeyPoints.Count; ++i)
            {
                _wrist.Add(CreateKeyPoint(Hand.Wrist.KeyPoints[i], _wristColor, 0).transform);
            }
#endif
        }
        ...
        private GameObject CreateKeyPoint(MLHandTracking.KeyPoint keyPoint, Color color, int index)
        {
            if (index == 1)
            {
                return thumb_1;
