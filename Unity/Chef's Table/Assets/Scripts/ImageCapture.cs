 ...
 //       private GameObject _previewObject = null;
        ...
        void Awake()
        {
           ...
           // _previewObject.SetActive(false);
            ...
            if (!result.IsOk)
            {
                Debug.LogErrorFormat("Error: ImageCapture failed starting MLPrivilegesStarterKit, disabling script. Reason: {0}", result);
                enabled = false;
                return;
            }
#endif
...
//#endif
    ...
    //            LocalizeManager.GetString(ControllerStatus.Text));
...
#if PLATFORM_LUMIN
                //MLInput.OnControllerButtonDown += OnButtonDown;
                // BEGIN CAPTURING STILL IMAGES AND SAVE TO DEVICE
                OnButtonDown();
#endif
        ...
        private void OnCaptureRawImageComplete(byte[] imageData)
        {
            ...
            //  Currently doesn't save captured image anywhere
            ...
            {
                texture.EncodeToPNG();
                Debug.Log("encoded to png success");
